...
| `POST`       | `/api/tickets/:id/submissions` | **O envio do composer**: uma parte de texto por canal, o status e a conclusão numa transação só, sob o `submissionId` que o cliente gera. Repetir o mesmo id devolve `200` com o envio já gravado; qualquer recusa não grava nada, nem o texto |
