...
As rotas de `/api/auth/*` estão em [Autenticação](#autenticação). Contrato completo em `openapi.json` (27 rotas, 38 operações).
...
| `GET`        | `/api/tickets/:id`             | Um chamado pelo id, com o `enrollmentSnapshot` completo, a conclusão e as pendências abertas (`openPendencies`: desde quando e quantas vezes foi cobrada), derivadas dos eventos                                                                                                                                                                                                                     |
...
| `POST`       | `/api/tickets/:id/submissions` | **O envio do composer**: uma parte de texto por canal, o status, a conclusão e as pendências marcadas numa transação só, sob o `submissionId` que o cliente gera. Cada ação sobre pendências (`opened`, `reopened`, `resolved`) vira um evento `pendency_changed` na cronologia, fora do envio. Repetir o mesmo id devolve `200` com o envio já gravado; qualquer recusa não grava nada, nem o texto |
...
| `GET`        | `/api/pendency-items`          | O catálogo de pendências que a analista cobra do RH, na ordem do drawer. `?enrollmentType=` traz só os itens daquele tipo e os que valem para qualquer tipo                                                                                                                                                                                                                                          |
...
| --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| `/api/tickets/**`, `/api/tickets/:id/comments`, `/api/tickets/:id/timeline`, `/api/queues/:id/tickets`, `GET /api/pendency-items` | `admin/allow/administrate/pipodesk/ticket`    |
